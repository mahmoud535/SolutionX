package com.example.solutionx.features.singleclick.domain.usecases

import android.content.Context
import com.example.solutionx.features.singleclick.domain.model.Countries
import com.example.solutionx.features.singleclick.domain.model.Currencies
import com.example.solutionx.features.singleclick.domain.repository.Repository
import javax.inject.Inject

//class GetAllCurrenciesUseCase @Inject constructor(private val repository: Repository) {
//    operator fun invoke(context: Context): List<Currencies> = repository.getAllCurrencies(context)
//
//}